...
Y = as.matrix(parliament[,.(catsp,cs,cup,erc,jxcat,pp,psc,other)])
...
Hzr = data.table(coordinates(Yzr, B, label = 'ilr'))
Hnz = coordinates(Ynz, B, label = 'ilr')



BASIS = as.data.table(t(sign(B)),keep.rownames=TRUE)
names(BASIS)[1]=  'Basis:'
BASIS %>%
  flextable() %>%
  colformat_int(col_keys = colnames(Y))

Mzr = Mclust(Hzr, modelNames = "EII", G=5:20)
Mnz = Mclust(Hnz, modelNames = "EII", G=5:20)
...
H_zr_s = c_rlrnm_mixture_posterior(500, Y[I,], PI_zr, MU_zr, SIGMA_zr, t(MASS::ginv(B)), 1000) %>% as.data.frame()
l_post_zr = data_posterior(xseq = seq(min(H_zr_s[,1]), max(H_zr_s[,1]), length.out = 100),
                           ...
                           Y[I,], PI_zr, MU_zr, SIGMA_zr, B) %>%
  lapply(as.data.table)
########
PI_nz = Mnz$parameters$pro
MU_nz = Mnz$parameters$mean
SIGMA_nz = Mnz$parameters$variance$sigma
H_nz_s = c_rlrnm_mixture_posterior(500, Y[I,], PI_nz, MU_nz, SIGMA_nz, t(MASS::ginv(B)), 1000) %>% as.data.frame()
...
if(FALSE){
  Hs_zr = apply(Y, 1, function(x_) c_rlrnm_mixture_posterior(1, x_, PI_zr, MU_zr, SIGMA_zr, t(MASS::ginv(B)), r = 100)) %>% 
    t() %>% as.data.table()
  Hs_nz = apply(Y, 1, function(x_) c_rlrnm_mixture_posterior(1, x_, PI_nz, MU_nz, SIGMA_nz, t(MASS::ginv(B)), r = 100)) %>% 
    t() %>% as.data.table()
...
save.image('IFCS2019/ifcs2019_all_dim.RData')


# m = fpc::discrcoord(as.matrix(Hnz), C_mv_nz)
# plot(m$proj, col = C_mv_nz)
